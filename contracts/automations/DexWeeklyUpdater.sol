...
contract DexWeeklyUpdater is ArcBaseWithRainbowRoad
{
    ...
    address axelarSender;
    address chainlinkSender;
    address layerZeroSender;
    ...
    function runWeeklyUpdates() external onlyAuthorized
    {
        for(uint256 i = 0; i < chains.length; i++) {
            try this.runWeeklyUpdate(chains[i]) {
                
            } catch {
                emit WeeklyUpdateNotProcessed(chains[i], isActive[chains[i]]);
            }
        }
    }
    
    function runWeeklyUpdate(uint256 chainId) public onlyAuthorized
    {
    ...
    function processWeeklyUpdate(uint256 chainId) internal
    {
        if(isActive[chainId]) {
            ...
            emit WeeklyUpdateSuccess(chainId, isActive[chainId], provider);
        } else {
            emit WeeklyUpdateNotProcessed(chainId, isActive[chainId]);
        }
    }
